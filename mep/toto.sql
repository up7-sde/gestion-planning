...
BEGIN
	-- Déclarer 2 variable
    DECLARE v_nbHeureCMAffectee INT;
    DECLARE v_nbHeureCM INT;
    DECLARE v_nbHeureCMTotal INT;
    -- Initialiser les 2 variables
	CALL calculerNbHeureCMAffecte(UPPER(p_apogee), v_nbHeureCMAffectee);
    SELECT `sde`.`Enseignements`.`heureCM` INTO v_nbHeureCM
    FROM `sde`.`Enseignements`
    WHERE `sde`.`Enseignements`.`apogee` = p_apogee;
    SET v_nbHeureCMTotal = v_nbHeureCMAffectee +  p_nbHeures;
    -- Insérer si le nombre d'heure de cours d'un enseignement ne dépasse pas le nb CM 
    IF v_nbHeureCMTotal <= v_nbHeureCMTotal THEN
...
-- -----------------------------------------------------
-- procedure calculerNbHeureCMAffecte
-- -----------------------------------------------------

USE `sde`;
DROP procedure IF EXISTS `sde`.`calculerNbHeureCMAffecte`;

DELIMITER $$
USE `sde`$$
CREATE PROCEDURE `calculerNbHeureCMAffecte` (IN p_apogee VARCHAR(45), OUT p_nbHeureCM INT)
BEGIN
	SELECT SUM(`sde`.`Cours`.`nbHeures`) INTO p_nbHeureCM
    FROM
		`sde`.`Cours`
	INNER JOIN
